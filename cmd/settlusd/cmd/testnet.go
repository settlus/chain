...
Example:
	settlusd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "settlusd", "Home directory of the node's daemon configuration")
...
Example:
	settlusd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		nodeIDs[i], valPubKeys[i], err = initializeNodeValidatorFilesFromMnemonic(nodeConfig, "")
		if err != nil {
			_ = os.RemoveAll(args.outputDir)
			return err
		}
		...
		accStakingTokens := sdk.TokensFromConsensusPower(100000000, settluscfg.PowerReduction)
		usdcStakingTokens := sdk.TokensFromConsensusPower(100000000, settluscfg.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(settluscfg.BaseDenom, accStakingTokens),
			sdk.NewCoin("uusdc", usdcStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, settluscfg.PowerReduction)
		...
		fee := sdk.NewCoins(sdk.NewCoin(settluscfg.BaseDenom, sdk.NewInt(210000000)))
		...
		customAppTemplate, customAppConfig := config.AppConfig(settluscfg.BaseDenom)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, settluscfg.BaseDenom, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
...
func initGenFiles(
	...
	genDoc.ConsensusParams.Block.MaxGas = 100000000
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		if err := exportGenDocWithTime(genFile, chainID, nil, appState, genTime, genDoc); err != nil {
			return err
		}
	}
...
func exportGenDocWithTime(
	genFile, chainID string, validators []types.GenesisValidator,
	appState json.RawMessage, genTime time.Time, genDoc *types.GenesisDoc,
) error {
