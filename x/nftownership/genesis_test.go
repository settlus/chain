...
func (suite *GenesisTestSuite) TestNftOwnershipInitGenesis() {
	testCases := []struct {
		name         string
	...
	for _, tc := range testCases {
		suite.Require().NotPanics(func() {
			nftownership.InitGenesis(suite.ctx, *suite.app.NftOwnershipKeeper, tc.genesisState)
		})
		params := suite.app.NftOwnershipKeeper.GetParams(suite.ctx)
