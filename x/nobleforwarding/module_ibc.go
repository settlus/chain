...
func (im IBCModule) OnRecvPacket(
	...
	var modulePacketData types.ForwardingAccountPacketData
	...
	case *types.ForwardingAccountPacketData_ForwardingAccountPacket:
		ack = channeltypes.NewErrorAcknowledgement(fmt.Errorf("settlus does not accept ica packets"))
	default:
...
func (im IBCModule) OnAcknowledgementPacket(
	...
	switch packet := modulePacketData.Packet.(type) {
	case *types.ForwardingAccountPacketData_ForwardingAccountPacket:
		if err := im.keeper.OnAcknowledgementForwardingPacket(ctx, modulePacket, *packet.ForwardingAccountPacket, ack); err != nil {
			return err
		}
		eventType = types.EventTypeForwardingAccountPacket
	default:
...
func (im IBCModule) OnTimeoutPacket(
	...
	switch packet := modulePacketData.Packet.(type) {
	case *types.ForwardingAccountPacketData_ForwardingAccountPacket:
		if err := im.keeper.OnTimeoutForwardingPacket(ctx, modulePacket, *packet.ForwardingAccountPacket); err != nil {
			return err
		}
	default:
		errMsg := fmt.Sprintf("unrecognized %s packet type: %T", types.ModuleName, packet)
