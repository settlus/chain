...
// OnAcknowledgementForwardingPacket processes packet reception
func (k Keeper) OnAcknowledgementForwardingPacket(ctx sdk.Context, packet channeltypes.Packet, data types.ForwardingAccountPacket, ack channeltypes.Acknowledgement) error {
	switch dispatchedAck := ack.Response.(type) {
	case *channeltypes.Acknowledgement_Error:
		ackErr := dispatchedAck.Error
	...
	case *channeltypes.Acknowledgement_Result:
		...
		var packetAck types.ForwardingAccountPacket

		if err := types.ModuleCdc.UnmarshalJSON(dispatchedAck.Result, &packetAck); err != nil {
			// The counter-party module doesn't implement the correct acknowledgment format
			return types.ErrInvalidPacketData
		}
	...
	default:
		// The counter-party module doesn't implement the correct acknowledgment format
		return types.ErrInvalidAckFormat
	}
}

// OnTimeoutForwardingPacket responds to the case where a packet has not been transmitted because of a timeout
func (k Keeper) OnTimeoutForwardingPacket(ctx sdk.Context, packet channeltypes.Packet, data types.ForwardingAccountPacket) error {
	...
	// TODO: should we retry the packet?
